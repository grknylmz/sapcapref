...
async function clearDB(db) {
    ...
    // ])
    console.log(`DB Tables Cleared` + db)
}
...
async function filmsLoad(db, films, people, planets, species, starships, vehicles) {
    ...
    await db.run(INSERT.into(db.entities.Film2People).entries(Film2PeopleNew))
    await db.run(INSERT.into(db.entities.Film2Planets).entries(Film2PlanetsNew))
    await db.run(INSERT.into(db.entities.Film2Starships).entries(Film2StarshipsNew))
    await db.run(INSERT.into(db.entities.Film2Vehicles).entries(Film2VehiclesNew))
    await db.run(INSERT.into(db.entities.Film2Species).entries(Film2SpeciesNew))
